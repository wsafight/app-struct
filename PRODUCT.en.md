...
Social login can now declare `modules.auth.providers: [google, github]`; `oauth: true` remains the generic OIDC shorthand. Generated applications can opt into Stripe Billing v1 (subscriptions, trials, customer portal, and idempotent signed webhooks); `appstruct capabilities --format json` reports the provider matrix and required environment variables.
