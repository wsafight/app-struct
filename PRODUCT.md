...
本文是产品基线，不是入门指南。要构建应用，请从 [概览](README.zh-CN.md) 开始。
...
社交登录现在支持按应用声明 `modules.auth.providers: [google, github]`，旧的 `oauth: true` 仍表示通用 OIDC。生成应用现在可按 Spec 启用 Stripe Billing v1（订阅、试用、客户门户和签名 Webhook 幂等处理）；使用 `appstruct capabilities --format json` 可查看 Provider 矩阵与必需环境变量。
...
每个生成资源提供带逐条授权、租户隔离、Policy/Hook、revision 和审计语义的批量更新/删除接口，以及 CSV 导入导出接口。React 列表支持勾选记录、批量更新、批量删除、CSV 下载和导入，部分失败会返回成功与失败明细供重试。
...
- `external` 要求通过进程环境或 `.env` 提供 `DATABASE_URL`，但不启动或停止数据库进程。
- `managed` 的迁移策略默认是 `prompt`，`external` 默认是 `unmanaged`。
- `auto` 自动创建并应用安全迁移；`prompt` 仅在存在 plan 或 pending 时询问；`never` 只读检查并阻止不一致 schema；`unmanaged` 跳过所有 AppStruct 迁移检查后直接启动。
- 生产后端始终不执行迁移；secret 不进入 App Spec、IR 或生成物。
