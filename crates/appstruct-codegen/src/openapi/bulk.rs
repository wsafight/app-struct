...
pub(super) fn add_paths(paths: &mut Map<String, Value>, _ir: &AppIr, entity: &EntityIr) {
    ...
    if entity.views.soft_delete {
        paths.insert(format!("{base}_trash"), json!({ "get": { "operationId": format!("list{name}Trash"), "tags": [name], "security": security, "responses": { "200": response("Trashed resources", &schema_ref("TrashResponse")), "403": error_response() } } }));
...
pub(super) fn add_schemas(schemas: &mut Map<String, Value>, entity: &EntityIr) {
    ...
    if entity.views.soft_delete {
        schemas.insert("TrashResponse".to_owned(), json!({ "type": "object", "required": ["data"], "properties": { "data": { "type": "array", "items": schema_ref(&entity.rust_name) } } }));
    }
}
