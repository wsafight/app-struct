...
export function ResourceDetail({ resource }: { resource: ResourceDefinition }) {
  ...
  return <main className="page detail-page">
    ...
    {record && <dl className="detail-grid">{resource.fields.filter((field) => canAccessRule(field.readAccess ?? { mode: "public" }, actor)).map((field) => <div key={field.name}><dt>{field.label}</dt><dd>{formatValue(record[field.name])}</dd></div>)}</dl>}
  </main>;
}
