...
import { ChevronLeft, ChevronRight, Download, Plus, RefreshCw, RotateCcw, Search, Trash2, Upload } from "lucide-react";
...
export function ResourceList({ resource, resources }: { resource: ResourceDefinition; resources: ResourceDefinition[] }) {
  ...
  useEffect(() => { setRowSelection({}); }, [controller.dataUpdatedAt]);
  useRealtimeResource({ enabled: controller.canList, resourceId: resource.id, resourceSlug: resource.slug, eventPrefix: resource.eventPrefix });
  ...
  async function updateField(record: ResourceRecord, field: FieldDefinition, value: unknown): Promise<boolean> {
  ...
  const bulk = useBulkActions({ resource, records, rowSelection, trashMode, actor, runChange, onError: setActionError });
  const pages = Math.max(1, Math.ceil(total / pageSize));
  const error = actionError || (controller.error ? errorMessage(controller.error) : "");
  const busy = controller.changing;
  if (!controller.canList) return <AccessDenied />;
  return <main className="page">
    <div className="page-heading">
      <div><h1>{trashMode ? `${resource.label} trash` : resource.label}</h1><p>{total} records</p></div>
      <div className="toolbar">
        {resource.softDelete && <button className="icon-button" onClick={() => updateParam("trash", trashMode ? undefined : "1")} title={trashMode ? "Show active records" : "Show trash"} aria-label={trashMode ? "Show active records" : "Show trash"}>{trashMode ? <RotateCcw size={17} /> : <Trash2 size={17} />}</button>}
        <button className="icon-button" onClick={csv.exportCsv} disabled={csv.exporting} title="Export CSV" aria-label="Export CSV"><Download size={17} /></button>
        {canCreate && !trashMode && <><input ref={csv.importInput} className="sr-only" type="file" accept=".csv,text/csv" onChange={(event) => void csv.importCsv(event.target.files?.[0])} /><button className="icon-button" onClick={() => csv.importInput.current?.click()} disabled={busy} title="Import CSV" aria-label="Import CSV"><Upload size={17} /></button></>}
        <button className="icon-button" onClick={() => void controller.refetch()} disabled={controller.fetching} title="Refresh" aria-label="Refresh"><RefreshCw size={17} /></button>
        {canCreate && !trashMode && <Link className="primary-button" to={`/${resource.slug}/new`}><Plus size={17} /> Add</Link>}
      </div>
    </div>
    ...
    <SavedViews resourceId={resource.id} actorId={actor?.id} onError={setActionError} />
    <BulkToolbar actions={bulk} trashMode={trashMode} busy={busy} />
    {error && <div className="alert" role="alert">{error}</div>}
    <ResourceTable resource={resource} actor={actor} records={records} visibleFields={visibleFields} sort={sort} trashMode={trashMode} pending={controller.pending} fetching={controller.fetching} rowSelection={rowSelection} setRowSelection={setRowSelection} changeSort={changeSort} updateField={updateField} remove={remove} restore={restoreOne} />
