...
export function useBulkActions({
  ...
  useEffect(() => {
    if (!writableFields.some((field) => field.name === bulkField))
  ...
  async function bulkUpdate() {
    const field = resource.fields.find(
      (candidate) => candidate.name === bulkField,
    );
    ...
    <div className="bulk-toolbar">
      ...
      {!trashMode && actions.writableFields.length > 0 && (
        <>
          <select
            ...
            value={actions.bulkField}
            onChange={(event) => actions.setBulkField(event.target.value)}
          >
            {actions.writableFields.map((field) => (
          ...
          <input
            ...
            value={actions.bulkValue}
            onChange={(event) => actions.setBulkValue(event.target.value)}
          />
          <button
            className="secondary-button"
            disabled={busy}
            onClick={() => void actions.bulkUpdate()}
          >
            <Check size={16} /> Apply
          </button>
        </>
      ...
      {trashMode ? (
        <button
          className="secondary-button"
          disabled={busy}
          onClick={() => void actions.restoreSelected()}
        >
        ...
        <button
          ...
          disabled={busy}
          onClick={() => void actions.bulkDelete()}
