...
interface ResourceTableProps {
  ...
  setRowSelection: (selection: RowSelectionState | ((current: RowSelectionState) => RowSelectionState)) => void;
  changeSort: (field: string) => void;
  updateField: (record: ResourceRecord, field: FieldDefinition, value: unknown) => Promise<boolean>;
  remove: (id: string) => Promise<void>;
  restore: (id: string) => Promise<void>;
}

export function ResourceTable({ resource, actor, records, visibleFields, sort, trashMode, pending, fetching, rowSelection, setRowSelection, changeSort, updateField, remove, restore }: ResourceTableProps) {
  const columns = [
    ...
    ...visibleFields.map((field) => resourceColumnHelper.accessor((record) => record[field.name], {
      ...
      cell: (info) => {
        ...
        const editable = !trashMode
          && supportsInlineEdit(field)
          && canAccessResource(resource, "update", actor, record)
          && canAccessRule(field.writeAccess ?? { mode: "public" }, actor, record);
        return editable
          ? <InlineEditor record={record} field={field} onSave={(value) => updateField(record, field, value)} />
    ...
    resourceColumnHelper.display({
      ...
      cell: ({ row }) => {
        ...
        return <div className="row-actions">
          {canRead && <Link className="icon-button" to={`/${resource.slug}/${encodeURIComponent(id)}`} title="View" aria-label="View"><Eye size={16} /></Link>}
          {trashMode
            ? <button className="icon-button" onClick={() => void restore(id)} title="Restore" aria-label="Restore"><RotateCcw size={16} /></button>
            : canDelete && <button className="icon-button danger" onClick={() => void remove(id)} title={resource.softDelete ? "Move to trash" : "Delete"} aria-label={resource.softDelete ? "Move to trash" : "Delete"}><Trash2 size={16} /></button>}
        </div>;
      },
    }),
  ];
  ...
  return <div className="table-frame"><table aria-busy={fetching}>
    <thead>{table.getHeaderGroups().map((headerGroup) => <tr key={headerGroup.id}>{headerGroup.headers.map((header) => <th key={header.id} className={header.column.id === "selection" ? "selection-cell" : undefined}>{header.isPlaceholder ? null : <table.FlexRender header={header} />}</th>)}</tr>)}</thead>
    <tbody>
      {pending && <tr><td colSpan={table.getAllLeafColumns().length} className="empty">Loading...</td></tr>}
      {!pending && table.getRowModel().rows.length === 0 && <tr><td colSpan={table.getAllLeafColumns().length} className="empty">No records</td></tr>}
      {!pending && table.getRowModel().rows.map((row) => <tr key={row.id}>{row.getAllCells().map((cell) => <td key={cell.id} className={cell.column.id === "selection" ? "selection-cell" : cell.column.id === "actions" ? "row-actions-cell" : undefined}><table.FlexRender cell={cell} /></td>)}</tr>)}
    </tbody>
  </table></div>;
}
