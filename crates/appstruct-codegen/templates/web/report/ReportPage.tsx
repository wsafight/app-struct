...
export function ReportPage() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: () =>
      client.invalidateQueries({ queryKey: appQueryKeys.reports.all }),
  });
        ...
        <div className="report-create-grid">
          ...
          <button
            ...
            disabled={
              !selectedTemplate ||
              create.isPending ||
              !requiredPresent(selectedTemplate, values)
            }
            onClick={() =>
              selectedTemplate &&
              create.mutate({ template: selectedTemplate.name, data: values })
            }
          >
            ...
            {(runs.data?.data ?? []).map((run) => (
              <tr key={run.id}>
                <td>
                  ...
                  <small>v{run.template_version}</small>
                </td>
                <td>{formatDate(run.created_at)}</td>
                <td>
                  <span className={`report-status ${run.stage}`}>
                    {run.stage}
                  </span>
                  {run.error_code && <small>{run.error_code}</small>}
                </td>
                <td>
                  <progress
                    max={100}
                ...
                <td>
                  <div className="row-actions">
                    {["queued", "rendering"].includes(run.stage) && (
                      <button
                        type="button"
                        className="icon-button"
                        title="Cancel report"
                        aria-label={`Cancel ${run.template}`}
                    ...
                    {run.stage === "succeeded" && (
                      <button
                        ...
                        className="icon-button"
                        title="Download PDF"
                        aria-label={`Download ${run.template}`}
...
function SchemaFields({
  ...
  const required = new Set(
    Array.isArray(template.input_schema.required)
...
function requiredPresent(
  ...
  const required = Array.isArray(template.input_schema.required)
    ? template.input_schema.required
    : [];
...
function active(run: ReportRun): boolean {
  return (
    run.stage === "queued" ||
    run.stage === "rendering" ||
