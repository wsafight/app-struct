...
fn audit_contract_generates_a_compilable_backend() {
    ...
    assert!(sql.contains("\"operation\" TEXT NOT NULL"));
    ...
    assert!(
        openapi["components"]["schemas"]["AuditEvent"]["properties"]["operation"]["pattern"]
            .is_string()
    );
    assert!(client.contains("operation: string"));
    assert!(client.contains("metadata: Record<string, unknown> | null"));
