...
fn expect_sequence<'node>(node: &'node Node, context: &str) -> Result<&'node [Node], Diagnostic> {
...
fn expect_string(node: &Node, context: &str) -> Result<Located<String>, Diagnostic> {
...
fn expect_scalar_string(node: &Node, context: &str) -> Result<Located<String>, Diagnostic> {
...
fn expect_bool(node: &Node, context: &str) -> Result<bool, Diagnostic> {
...
fn expect_u64(node: &Node, context: &str) -> Result<Located<u64>, Diagnostic> {
    let Some((value, true)) = node.scalar() else {
        return Err(Diagnostic::error(
            "AS1007",
...
fn optional_bool(mapping: &BTreeMap<String, MappingEntry>, key: &str) -> Result<bool, Diagnostic> {
    mapping
        .get(key)
