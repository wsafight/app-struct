...
启用 Auth 的项目包含 `/admin` 页面和匹配的 `GET /api/admin/overview` 端点。用它查看健康、队列和投递，并执行受保护的恢复操作。这是运维面，不是第二套应用 UI。该端点仅限具有已配置 `admin` 角色的操作者，并报告用户、会话、组织、邀请、排队/死信 Jobs、Mail 投递、Files 和 Audit 事件的实时计数。
