...
test("dashboard registration and project lifecycle", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("banner").getByText(email, { exact: true }),
  ).toBeVisible();
...
test("authentication screen fits a mobile viewport", async ({
  page,
}, testInfo) => {
